...
# Patterns that indicate Google/MyMemory returned an error page instead of a translation
_GOOGLE_ERROR_PATTERNS = (
    'Error 5',          # HTTP 500 errors
    'Error 4',          # HTTP 400/404 errors
    "That's an error",  # Google error page text
    'Please try again', # Google error page text
    '<html',            # Any raw HTML
    '<!DOCTYPE',        # Any raw HTML
)

_TRANSLATE_LOCK = __import__('threading').Lock()
_LAST_REQUEST_TIME = [0.0]   # shared across threads
_MIN_INTERVAL = 0.35         # minimum seconds between any two API calls

def _throttle():
    """Ensure at least _MIN_INTERVAL seconds between successive API calls."""
...
def _try_google(text: str, lang_code: str):
    """Tier 2 — Google Translate via deep-translator (with throttle + retry)."""
    from deep_translator import GoogleTranslator
    for attempt in range(3):
        try:
            ...
            if out:
                out = out.strip()
                if out and not any(pat in out for pat in _GOOGLE_ERROR_PATTERNS):
                    return (out, "Google")
            # Got an error page — back off before retrying
            _time.sleep(1.5 + attempt * 1.5 + _random.uniform(0, 0.5))
        except Exception:
...
def _try_mymemory(text: str, lang_name: str, lang_code: str):
    """Tier 3 — MyMemory via deep-translator (with throttle + retry)."""
    from deep_translator import MyMemoryTranslator
    for attempt in range(2):
        try:
            _throttle()
            target = "en-" + lang_code.upper() if len(lang_code) == 2 else lang_code
...
def encode_for_file(text: str, lang_name: str) -> str:
    ...
    For all languages: return the actual text characters (will be written as UTF-8).
    Arabic/Hindi: returns actual Arabic/Hindi characters (not escape sequences).
    Others:       returns actual language characters.
    """
    # Retain the padding backticks as they are required for length consistency in .h file compilation
    return text
MAX_WORKERS     = 2   # keep low to avoid Google rate-limiting (parallel requests trigger 500 errors)
AUTO_FREE_SLOTS = 4
...
def pad_or_trim(text, length, lang_name=""):
    """
    ...
    w = get_width(text, lang_name)
    if w >= length:
        if lang_name.lower() == 'hindi':
            return text
        return text[:length]
    else:
        return text + BACKTICK * (length - w)
...
def _do_translate(text: str, lang_name: str) -> tuple:
    """
    Translate text into lang_name.
    - DeepL is the PRIMARY translator for all languages it supports.
    - Google / MyMemory are used ONLY as fallback for languages DeepL
      cannot handle (Arabic, Hindi, Vietnamese, Indonesian).
    ...
    if nav_langs:
        ...
        for entry in translatable:
            ...
            if result:
                ...
                new_block_lines.append(build_line('DEFSTR', pad_or_trim(translated, str_length, lang_name), lang_name) + eol)
            else:
        ...
        for ln, lc in lang_slots:
            ...
            if result:
                translated = result[0] if isinstance(result, tuple) else result
                new_block_lines.append(build_line('DEFSTR', pad_or_trim(translated, str_length, ln), ln) + eol)
            else:
...
def _do_translate_batch(texts: list, lang_name: str) -> dict:
    """
    Translates a list of texts (batch) into lang_name.
    Returns a dict mapping original_text -> (translated_str, tier).
    """
    results = {}
    bodies = []
    prefix_map = {}
    ...
    for t in texts:
        t_strip = t.strip()
        if not t_strip or all(c == BACKTICK for c in t_strip):
            results[t] = ("", "skip")
            continue
        prefix = ""
        body = t_strip
        ...
        if body not in prefix_map:
            prefix_map[body] = []
        prefix_map[body].append((t, prefix))
    ...
    # Tier 1: DeepL
    if lang_name not in DEEPL_UNSUPPORTED:
        dl_code = DEEPL_LANG_MAP.get(lang_name)
        translator = _get_deepl_translator()
        if translator and dl_code:
            try:
                dl_res = translator.translate_text(bodies, target_lang=dl_code)
                for i, res in enumerate(dl_res):
                    out = res.text.strip()
                    if out:
                        translated_bodies[bodies[i]] = (out, "DeepL")
            except Exception:
                pass

    # Tier 2: Google Translate (Batching with individual fallback)
    untranslated = [b for b in bodies if b not in translated_bodies]
    if untranslated:
        try:
            from deep_translator import GoogleTranslator
            batch = "\n".join(untranslated)
            out = GoogleTranslator(source='auto', target=lang_code).translate(batch)
            if out and not any(pat in out for pat in _GOOGLE_ERROR_PATTERNS):
                res_parts = [p.strip() for p in out.split("\n")]
                if len(res_parts) == len(untranslated):
                    for i, r in enumerate(res_parts):
                        if r: translated_bodies[untranslated[i]] = (r, "Google")
                else:
                    raise ValueError("Batch length mismatch")
        except Exception:
            # Fallback to translating individually if batch fails
            for b in untranslated:
                try:
                    from deep_translator import GoogleTranslator
                    out = GoogleTranslator(source='auto', target=lang_code).translate(b)
                    if out and not any(pat in out for pat in _GOOGLE_ERROR_PATTERNS):
                        translated_bodies[b] = (out.strip(), "Google (indiv)")
                except Exception:
                    pass
    ...
    if untranslated:
        ...
        for b in untranslated:
            try:
                out = MyMemoryTranslator(source='en-US', target=target).translate(b)
                if out:
                    out = out.strip()
                    if out and not any(pat in out for pat in _GOOGLE_ERROR_PATTERNS):
                        translated_bodies[b] = (out, "MyMemory")
            except Exception:
                pass

    # Reassemble results with prefixes
    for body in bodies:
        if body in prefix_map:
            for orig_text, prefix in prefix_map[body]:
                if body in translated_bodies:
                    trans, tier = translated_bodies[body]
                    results[orig_text] = (prefix + trans, tier)
                else:
                    results[orig_text] = (orig_text, "failed")
...
def translate_parallel(unique_texts, lang_code, lang_name, stop_event, on_one_done=None):
    ...
    # Process in chunks of 15 for safety (preventing API request-size limits)
    chunk_size = 15
    chunks = [unique_texts[i:i+chunk_size] for i in range(0, len(unique_texts), chunk_size)]
    
    for chunk in chunks:
        if stop_event.is_set(): break
        batch_results = _do_translate_batch(chunk, lang_name)
        for t in chunk:
            trans_tuple = batch_results.get(t, (t, "failed"))
            results[t] = trans_tuple
            done_count[0] += 1
            if on_one_done: on_one_done(done_count[0], total, t)
...
def process_file(filepath, lang_name, lang_code, extra_english="",
    ...
    if not blocks_to_translate and lang_already_implemented and extra_eng:
        ...
        fitted = encode_for_file(pad_or_trim(translated_extra, ref_len, lang_name), lang_name)
    ...
    for block in blocks_to_translate:
        ...
        fitted     = encode_for_file(pad_or_trim(translated, block['str_len'], lang_name), lang_name)
                ...
                if cmd == 'log':
                    ...
                    if stats.get('extra_already_added'):
                        ...
                        self.log.insert('end',
                            ...
                            f"   No changes were made.\n", 'ok')
                        self.log.see('end')
                        messagebox.showerror("Already Translated",
